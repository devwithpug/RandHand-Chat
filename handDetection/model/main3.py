...
class Gesture_Predict:
    ...
    def __init__(self, user_dict: Dict, limit=1):
        ...
        for key in user_dict:
            decoded = np.asarray(bytearray(base64.b64decode(user_dict[key])), dtype=np.uint8)
            img = cv2.imdecode(decoded, cv2.IMREAD_COLOR)
            img_list.append(img)

        self.user_id = list(user_dict.keys())
    ...
    def compute_angles(self):
        ...
        for idx, file in enumerate(self.img_list):
            ...
            if result.multi_hand_landmarks is not None:
                for res in result.multi_hand_landmarks:
                    ...
                    self.angle_data.loc[str(self.user_id[idx])] = angle
...
for img_name in IMAGE_FILES:
    with open(img_name,'rb') as img:
        ...
        test_dict[img_name] = base64_string

gesture = Gesture_Predict(test_dict, limit=1.5)
